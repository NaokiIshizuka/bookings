...
func (m *Repository) PostReservation(w http.ResponseWriter, r *http.Request) {
	...
	//send notifications - first to quest
...
func (m *Repository) AdminShowReservation(w http.ResponseWriter, r *http.Request) {
	expload := strings.Split(r.RequestURI, "/")
	id, err := strconv.Atoi(expload[4])
	if err != nil {
		helpers.ServerError(w, err)
		return
	}
...
func (m *Repository) AdminReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	// assume that is no month/year specified
	now := time.Now()
	...
	for _, x := range rooms {
		...
		//get all the restrictions for the current from
...
func (m *Repository) AdminProcessReservation(w http.ResponseWriter, r *http.Request) {
	...
	m.App.Session.Put(r.Context(), "flash", "Reservation marked no-processed")
...
func (m *Repository) AdminPostReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	...
	for _, x := range rooms {
		...
		// that does not exist in our posted data, and if the restrictions id > 0, then it is a block we need to
		// remove.
		curMap := m.App.Session.Get(r.Context(), fmt.Sprintf("block_map_%d", x.ID)).(map[string]int)
		for name, value := range curMap {
			// ok will br fault if the value is not in the mao
	...
	for name, _ := range r.PostForm {
		if strings.HasPrefix(name, "add_block") {
			expload := strings.Split(name, "_")
			roomID, _ := strconv.Atoi(expload[2])
			t, _ := time.Parse("2006-01-2", expload[3])
			// insert a new block
